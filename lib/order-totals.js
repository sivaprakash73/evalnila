...
export function calculateCartTotals(items = [], shippingSettings = {}) {
  ...
  const taxableSubtotal = roundMoney(
    items.reduce(
      (sum, item) => sum + calculateTaxIncludedBreakup(item.price, item.taxPercentage ?? item.tax_percentage, item.quantity).taxableAmount,
      0
    )
  );
  const taxAmount = roundMoney(
    items.reduce(
      (sum, item) => sum + calculateLineTax(item.price, item.taxPercentage ?? item.tax_percentage, item.quantity),
      0
    )
  );
