...
function normalizeProduct(product) {
  ...
  const sizeDetails = normalizeSizeDetails(product.sizeDetails || product.size_details || []);
  const addons = normalizeProductAddons(product.addons || product.addonDetails || product.addon_details || []);
...
export async function getProducts(runQuery) {
  ...
  const rows = await runQuery(`
    SELECT
      ...
      (
        SELECT CONCAT('[', GROUP_CONCAT(JSON_OBJECT('label', s.label, 'bust', s.bust, 'waist', s.waist, 'hip', s.hip) ORDER BY s.sort_order ASC, s.label ASC SEPARATOR ','), ']')
        FROM product_sizes ps
        INNER JOIN sizes s ON s.id = ps.size_id
        WHERE ps.product_id = p.id
      ) AS sizeDetails,
      (
        SELECT CONCAT('[', GROUP_CONCAT(JSON_OBJECT('id', a.id, 'name', a.name, 'price', a.price) ORDER BY a.name ASC SEPARATOR ','), ']')
        FROM product_addons pa
        INNER JOIN addons a ON a.id = pa.addon_id
        WHERE pa.product_id = p.id AND a.is_active = 1
      ) AS addons,
      CASE
...
export async function createProduct(runQuery, payload) {
  ...
  const normalizedAddonIds = normalizeAddonIds(addons);
  ...
  await saveProductAddons(runQuery, productId, normalizedAddonIds);
...
export async function updateProduct(runQuery, id, payload) {
  ...
  await saveProductAddons(runQuery, Number(id), normalizedAddonIds);
...
function normalizeSizeDetails(value) {
  const details = parseSizeDetails(value);

  return details
    .map((item) => ({
      label: item?.label,
      bust: normalizeNullableNumber(item?.bust),
      waist: normalizeNullableNumber(item?.waist),
      hip: normalizeNullableNumber(item?.hip)
    }))
    .filter((item) => item.label);
}

function normalizeProductAddons(value) {
  const addons = parseSizeDetails(value);

  return addons
    .map((item) => ({
      id: Number(item?.id),
      name: item?.name,
      price: normalizeNullableNumber(item?.price) || 0
    }))
...
function normalizeAddonIds(value) {
  ...
  return [...new Set(items.map((item) => Number(typeof item === 'object' ? item.id : item)).filter((id) => Number.isInteger(id) && id > 0))];
}

function parseSizeDetails(value) {
  if (!value) {
    return [];
  }
...
async function saveProductAddons(runQuery, productId, addonIds) {
  await ensureAddonsTables(runQuery);
  await runQuery(`DELETE FROM product_addons WHERE product_id = ?`, [productId]);
